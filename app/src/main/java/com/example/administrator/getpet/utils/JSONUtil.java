...
public class JSONUtil {
     ...
     *
     * **/
    private static void serializeDate(JSONStringer js, Object obj) {
        try {
            js.value(TimeUtils.dateToString((Date)obj,TimeUtils.FORMAT_DATE_TIME_SECOND));
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    private static void setField(Object obj, Field f, JSONObject jo) {
        ...
        try {
            if (isArray(clazz)) { // 数组
                ...
                Object o = jo.opt(name);
                if (o != null) {
                    Date date = TimeUtils.stringToDate(o.toString(),TimeUtils.FORMAT_DATE_TIME_SECOND);
                    f.set(obj,date) ;
                }
            }
            else if (isObject(clazz)) { // 对象
