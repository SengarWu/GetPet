...
public abstract class BaseListAdapter<E> extends BaseAdapter {//BaseAdapter（基础适配器）是用于给listView，GridView等填充数据的
	...
	// adapter中的内部点击事件
	public Map<Integer, onInternalClickListener> canClickItem;//Map用于指示一个指定值对应一个指定值

	private void addInternalClickListener(final View itemV,//为列表中的每一项设置点击事件
										  final Integer position, final Object valuesMap) {
		if (canClickItem != null) {
			for (Integer key : canClickItem.keySet()) {//keySet()获取Map的所有key值
